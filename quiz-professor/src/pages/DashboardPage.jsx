...
export default function DashboardPage() {
  ...
  if (!alunoNome) {
    return <div className="text-center mt-20 text-gray-600 dark:text-gray-400">Carregando...</div>;
  }
  ...
  return (
    <div className="flex min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="flex-1 flex flex-col">
        <header className="md:hidden bg-white dark:bg-gray-800 shadow-sm px-4 py-3 flex items-center justify-between border-b border-gray-200 dark:border-gray-700 sticky top-0 z-30">
          <button onClick={() => setSidebarOpen(true)} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition">
            <div className="w-6 h-5 flex flex-col justify-between">
              <span className="h-0.5 w-6 bg-indigo-600 dark:bg-indigo-400 rounded-full"></span>
              <span className="h-0.5 w-6 bg-indigo-600 dark:bg-indigo-400 rounded-full"></span>
              <span className="h-0.5 w-6 bg-indigo-600 dark:bg-indigo-400 rounded-full"></span>
            </div>
          </button>
          <h1 className="text-lg font-serif font-bold text-indigo-800 dark:text-indigo-400">Prof. Marleta</h1>
